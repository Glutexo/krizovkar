...
from krizovkar.languages.czech import (
    ...
    unbreakable_word_boundaries,
)


class CzechTypographyTest(unittest.TestCase):
    def test_protects_lowercase_consonant_prepositions(self) -> None:
        self.assertEqual(
            ...
            protect_prepositions("v Praze, k řece, s přáteli a z domu"),
        )
    ...
    def test_hyphenates_word_joined_to_preposition(self) -> None:
        marked = mark_hyphenation(protect_prepositions("v Československu"))
    ...
    def test_marks_unbreakable_preposition_boundaries(self) -> None:
        self.assertEqual(
            frozenset((2, 5)),
            unbreakable_word_boundaries(
                ("JDU", "V", "LESE", "A", "Z", "DOMU")
